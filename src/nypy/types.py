...
import nypy.nodes
from nypy.bogus_type import Bogus
...
from nypy.util import IdMapper
...
class TypeAliasType(Type):
    ...
    def __init__(
        self,
        alias: nypy.nodes.TypeAlias | None,
        args: list[Type],
        line: int = -1,
        column: int = -1,
    ) -> None:
...
NOT_READY: Final = nypy.nodes.FakeInfo("De-serialization failure: TypeInfo not fixed")
...
class FunctionLike(ProperType):
    ...
    @abstractmethod
    def type_object(self) -> nypy.nodes.TypeInfo:
        pass
...
def get_proper_types(
    ...
    if isinstance(types, list):
        ...
        if not any(
            isinstance(t, (TypeAliasType, TypeGuardedType))
            for t in typelist  # type: ignore[misc]
        ):
...
from nypy.expandtype import ExpandTypeVisitor
