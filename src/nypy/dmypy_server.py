...
import nypy.errors
import nypy.main
from nypy.dmypy_util import WriteToConn, receive, send
from nypy.find_sources import InvalidSourceList, create_source_list
from nypy.fscache import FileSystemCache
from nypy.fswatcher import FileData, FileSystemWatcher
from nypy.inspections import InspectionEngine
from nypy.ipc import IPCServer
from nypy.modulefinder import BuildSource, FindModuleCache, SearchPaths, compute_search_paths
from nypy.options import Options
from nypy.server.update import FineGrainedBuildManager, refresh_suppressed_submodules
from nypy.suggestions import SuggestionEngine, SuggestionFailure
from nypy.typestate import reset_global_state
from nypy.util import FancyFormatter, count_stats
...
if sys.platform == "win32":
    ...
    def daemonize(
        ...
        """
        command = [sys.executable, "-m", "nypy.dmypy", "--status-file", status_file, "daemon"]
...
def process_start_options(flags: list[str], allow_sources: bool) -> Options:
    _, options = nypy.main.process_options(
...
class Server:
        ...
        try:
            ...
            with redirect_stderr(stderr):
                with redirect_stdout(stdout):
                    sources, options = nypy.main.process_options(
                        ["-i"] + list(args),
            ...
            if self.fine_grained_manager:
                ...
                _, current_plugins_snapshot = nypy.build.load_plugins(
    ...
    def initialize_fine_grained(
        ...
        t1 = time.time()
        try:
            result = nypy.build.build(sources=sources, options=self.options, fscache=self.fscache)
    ...
    def direct_imports(
        self, module: tuple[str, str], graph: nypy.build.Graph
    ) -> list[BuildSource]:
    ...
    def find_added_suppressed(
        self, graph: nypy.build.Graph, seen: set[str], search_paths: SearchPaths
    ) -> list[tuple[str, str]]:
...
def find_all_sources_in_build(
    graph: nypy.build.Graph, extra: Sequence[BuildSource] = ()
) -> list[BuildSource]:
    result = list(extra)
...
def fix_module_deps(graph: nypy.build.Graph) -> None:
