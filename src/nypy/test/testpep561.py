...
import nypy.api
from nypy.test.config import package_path, pip_lock, pip_timeout, test_temp_dir
from nypy.test.data import DataDrivenTestCase, DataSuite
from nypy.test.helpers import assert_string_arrays_equal, perform_file_operations
...
def test_pep561(testcase: DataDrivenTestCase) -> None:
    ...
    with virtualenv(python) as venv:
        ...
        for i, operations in enumerate(steps):
            ...
            out, err, returncode = nypy.api.run(cmd_line)
...
def test_mypy_path_is_respected() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            mypy_config_path = os.path.join(temp_dir, "nypy.ini")
            ...
            with virtualenv() as venv:
                ...
                out, err, returncode = nypy.api.run(cmd_line_args)
                assert returncode == 0
        finally:
            os.chdir(old_dir)
