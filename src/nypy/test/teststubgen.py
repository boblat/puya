...
from nypy.moduleinspect import InspectError, ModuleInspect
from nypy.stubdoc import (
    ArgSig,
    FunctionSig,
    build_signature,
...
from nypy.stubgenc import InspectionStubGenerator, infer_c_method_args
...
from nypy.test.helpers import assert_equal, assert_string_arrays_equal, local_sys_path_set
...
class StubgenCliParseSuite(unittest.TestCase):
    def test_walk_packages(self) -> None:
        with ModuleInspect() as m:
            assert_equal(set(walk_packages(m, ["nypy.errors"])), {"nypy.errors"})

            assert_equal(
                set(walk_packages(m, ["nypy.errors", "nypy.stubgen"])),
                {"nypy.errors", "nypy.stubgen"},
            )

            all_mypy_packages = set(walk_packages(m, ["mypy"]))
            self.assertTrue(
                all_mypy_packages.issuperset(
                    {"mypy", "nypy.errors", "nypy.stubgen", "nypy.test", "nypy.test.helpers"}
                )
            )
