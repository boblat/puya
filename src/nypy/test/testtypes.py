...
from nypy.erasetype import erase_type, remove_instance_last_known_values
from nypy.indirection import TypeIndirectionVisitor
from nypy.join import join_simple, join_types
from nypy.meet import meet_types, narrow_declared_type
...
from nypy.plugins.common import find_shallow_matching_overload_item
from nypy.state import state
from nypy.subtypes import is_more_precise, is_proper_subtype, is_same_type, is_subtype
from nypy.test.helpers import Suite, assert_equal, assert_type, skip
from nypy.test.typefixture import InterfaceTypeFixture, TypeFixture
from nypy.typeops import false_only, make_simplified_union, true_only
...
import nypy.expandtype  # ruff: isort: skip
...
class TypeOpsSuite(Suite):
    ...
    def assert_expand(
        ...
        exp = nypy.expandtype.expand_type(orig, lower_bounds)
...
class TestExpandTypeLimitGetProperType(TestCase):
    ...
    @skipUnless(nypy.expandtype.__file__.endswith(".py"), "Skip for compiled mypy")
    def test_count_get_proper_type(self) -> None:
        with open(nypy.expandtype.__file__) as f:
            code = f.read()
