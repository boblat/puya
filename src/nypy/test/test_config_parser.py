...
from nypy.config_parser import _find_config_file
from nypy.defaults import CONFIG_NAMES, SHARED_CONFIG_NAMES
...
class FindConfigFileSuite(unittest.TestCase):
    ...
    def test_precedence(self) -> None:
        with tempfile.TemporaryDirectory() as _tmpdir:
            ...
            mypy_ini = tmpdir / "nypy.ini"
            dot_mypy = tmpdir / ".nypy.ini"
            ...
            for cwd in [tmpdir, child]:
                ...
                with chdir(cwd):
                    result = _find_config_file()
                    assert result is not None
                    assert os.path.basename(result[2]) == "nypy.ini"
