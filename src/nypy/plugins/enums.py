...
from nypy.subtypes import is_equivalent
from nypy.typeops import fixup_partial_type, make_simplified_union
...
def enum_name_callback(ctx: nypy.plugin.AttributeContext) -> Type:
...
def _infer_value_type_with_auto_fallback(
    ctx: nypy.plugin.AttributeContext, proper_type: ProperType | None
) -> Type | None:
...
def enum_member_callback(ctx: nypy.plugin.FunctionContext) -> Type:
...
def enum_value_callback(ctx: nypy.plugin.AttributeContext) -> Type:
