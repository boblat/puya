...
import nypy.plugin  # To avoid circular imports.
from nypy.applytype import apply_generic_arguments
from nypy.errorcodes import LITERAL_REQ
from nypy.expandtype import expand_type, expand_type_by_instance
from nypy.exprtotype import TypeTranslationError, expr_to_unanalyzed_type
from nypy.meet import meet_types
from nypy.messages import format_type_bare
...
from nypy.plugin import SemanticAnalyzerPluginInterface
from nypy.plugins.common import (
    _get_argument,
    _get_bool_argument,
...
from nypy.server.trigger import make_wildcard_trigger
from nypy.state import state
from nypy.typeops import get_type_vars, make_simplified_union, map_type_from_supertype
...
from nypy.typevars import fill_typevars
from nypy.util import unmangle
...
class Attribute:
    ...
    def argument(self, ctx: nypy.plugin.ClassDefContext) -> Argument:
...
def _determine_eq_order(ctx: nypy.plugin.ClassDefContext) -> bool:
    """
...
def _get_decorator_optional_bool_argument(
    ctx: nypy.plugin.ClassDefContext, name: str, default: bool | None = None
) -> bool | None:
...
def attr_tag_callback(ctx: nypy.plugin.ClassDefContext) -> None:
...
def attr_class_maker_callback(
    ctx: nypy.plugin.ClassDefContext,
...
def _get_frozen(ctx: nypy.plugin.ClassDefContext, frozen_default: bool) -> bool:
...
def _analyze_class(
    ctx: nypy.plugin.ClassDefContext, auto_attribs: bool | None, kw_only: bool
) -> list[Attribute]:
...
def _detect_auto_attribs(ctx: nypy.plugin.ClassDefContext) -> bool:
...
def _attributes_from_assignment(
    ctx: nypy.plugin.ClassDefContext, stmt: AssignmentStmt, auto_attribs: bool, kw_only: bool
) -> Iterable[Attribute]:
...
def _parse_converter(
    ctx: nypy.plugin.ClassDefContext, converter_expr: Expression | None
) -> Converter | None:
    ...
    if isinstance(converter_expr, RefExpr) and converter_expr.node:
        if isinstance(converter_expr.node, FuncDef):
            ...
            from nypy.checkmember import type_object_type  # To avoid import cycle.
...
def _add_order(ctx: nypy.plugin.ClassDefContext, adder: MethodAdder) -> None:
...
def _make_frozen(ctx: nypy.plugin.ClassDefContext, attributes: list[Attribute]) -> None:
...
def _add_attrs_magic_attribute(
    ctx: nypy.plugin.ClassDefContext, attrs: list[tuple[str, Type | None]]
) -> None:
...
def _add_slots(ctx: nypy.plugin.ClassDefContext, attributes: list[Attribute]) -> None:
...
def _add_match_args(ctx: nypy.plugin.ClassDefContext, attributes: list[Attribute]) -> None:
    if (
...
def _remove_hashability(ctx: nypy.plugin.ClassDefContext) -> None:
...
class MethodAdder:
    ...
    def __init__(self, ctx: nypy.plugin.ClassDefContext) -> None:
        self.ctx = ctx
...
def _fail_not_attrs_class(ctx: nypy.plugin.FunctionSigContext, t: Type, parent_t: Type) -> None:
...
def _get_expanded_attr_types(
    ctx: nypy.plugin.FunctionSigContext,
    typ: ProperType,
...
def evolve_function_sig_callback(ctx: nypy.plugin.FunctionSigContext) -> CallableType:
    """
...
def fields_function_sig_callback(ctx: nypy.plugin.FunctionSigContext) -> CallableType:
