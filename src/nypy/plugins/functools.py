...
import nypy.plugin
import nypy.semanal
from nypy.argmap import map_actuals_to_formals
from nypy.nodes import ARG_POS, ARG_STAR2, ArgKind, Argument, CallExpr, FuncItem, NameExpr, Var
from nypy.plugins.common import add_method_to_class
from nypy.typeops import get_all_type_vars
...
def functools_total_ordering_maker_callback(
    ctx: nypy.plugin.ClassDefContext, auto_attribs_default: bool = False
) -> bool:
...
def _analyze_class(ctx: nypy.plugin.ClassDefContext) -> dict[str, _MethodInfo | None]:
...
def partial_new_callback(ctx: nypy.plugin.FunctionContext) -> Type:
    """Infer a more precise return type for functools.partial"""
    if not isinstance(ctx.api, nypy.checker.TypeChecker):  # use internals
...
def handle_partial_with_callee(ctx: nypy.plugin.FunctionContext, callee: Type) -> Type:
    if not isinstance(ctx.api, nypy.checker.TypeChecker):  # use internals
        return ctx.default_return_type
    ...
    for i, actuals in enumerate(formal_to_actual):
        if len(bound.arg_types) == len(fn_type.arg_types):
            ...
            if not nypy.checker.is_valid_inferred_type(arg_type):
    ...
    if not nypy.checker.is_valid_inferred_type(ret_type):
...
def partial_call_callback(ctx: nypy.plugin.MethodContext) -> Type:
    """Infer a more precise return type for functools.partial.__call__."""
    if (
        not isinstance(ctx.api, nypy.checker.TypeChecker)  # use internals
