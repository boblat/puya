...
import nypy.subtypes
import nypy.typeops
from nypy.argmap import ArgTypeExpander
from nypy.erasetype import erase_typevars
...
from nypy.types_utils import is_union_with_any
from nypy.typestate import type_state

if TYPE_CHECKING:
    from nypy.infer import ArgumentInferContext
...
def _infer_constraints(
    ...
    if isinstance(template, UnionType):
        template = nypy.typeops.make_simplified_union(template.items, keep_erased=True)
    if isinstance(actual, UnionType):
        actual = nypy.typeops.make_simplified_union(actual.items, keep_erased=True)
...
def infer_constraints_if_possible(
    ...
    """
    if direction == SUBTYPE_OF and not nypy.subtypes.is_subtype(erase_typevars(template), actual):
        return None
    if direction == SUPERTYPE_OF and not nypy.subtypes.is_subtype(
    ...
    if (
        ...
        and not nypy.subtypes.is_subtype(actual, erase_typevars(template.upper_bound))
    ):
...
def filter_satisfiable(option: list[Constraint] | None) -> list[Constraint] | None:
    ...
    for c in option:
        if isinstance(c.origin_type_var, TypeVarType) and c.origin_type_var.values:
            if any(
                nypy.subtypes.is_subtype(c.target, value) for value in c.origin_type_var.values
            ):
                satisfiable.append(c)
        elif nypy.subtypes.is_subtype(c.target, c.origin_type_var.upper_bound):
            satisfiable.append(c)
    if not satisfiable:
        return None
    return satisfiable
...
def is_same_constraint(c1: Constraint, c2: Constraint) -> bool:
    ...
    return (
        ...
        and nypy.subtypes.is_same_type(c1.target, c2.target)
    )
...
class ConstraintBuilderVisitor(TypeVisitor[list[Constraint]]):
    ...
    def visit_instance(self, template: Instance) -> list[Constraint]:
        ...
        if isinstance(actual, (CallableType, Overloaded)) and template.type.is_protocol:
            if "__call__" in template.type.protocol_members:
                ...
                if not any(template == t for t in template.type.inferring):
                    ...
                    call = nypy.subtypes.find_member(
                        "__call__", template, actual, is_operator=True
                    )
                    assert call is not None
                    if nypy.subtypes.is_subtype(actual, erase_typevars(call)):
        ...
        if isinstance(actual, CallableType) and actual.fallback is not None:
            if actual.is_type_obj() and template.type.is_protocol:
                ...
                if isinstance(ret_type, TupleType):
                    ret_type = nypy.typeops.tuple_fallback(ret_type)
        ...
        if isinstance(actual, Instance):
            ...
            if (
                ...
                and nypy.subtypes.is_protocol_implementation(instance, erased, skip=["__call__"])
            ):
                template.type.inferring.append(template)
                res.extend(
                    self.infer_constraints_from_protocol_members(
                ...
                and nypy.subtypes.is_protocol_implementation(erased, instance, skip=["__call__"])
            ):
        ...
        if (
            ...
            return infer_constraints(template, nypy.typeops.tuple_fallback(actual), self.direction)
    ...
    def infer_constraints_from_protocol_members(
        ...
        for member in protocol.type.protocol_members:
            inst = nypy.subtypes.find_member(member, instance, subtype, class_obj=class_obj)
            temp = nypy.subtypes.find_member(member, template, subtype)
            ...
            if nypy.subtypes.IS_SETTABLE in nypy.subtypes.get_member_flags(member, protocol):
...
def find_matching_overload_item(overloaded: Overloaded, template: CallableType) -> CallableType:
    ...
    for item in items:
        ...
        # subtype check.
        if nypy.subtypes.is_callable_compatible(
            item,
            template,
            is_compat=nypy.subtypes.is_subtype,
...
def infer_callable_arguments_constraints(
    ...
    for right_arg in right.formal_arguments():
        left_arg = nypy.typeops.callable_corresponding_argument(left, right_arg)
        if left_arg is None:
            continue
