...
from nypy.checkmember import analyze_member_access
from nypy.expandtype import expand_type_by_instance
from nypy.join import join_types
from nypy.literals import literal_hash
from nypy.maptype import map_instance_to_supertype
from nypy.meet import narrow_declared_type
from nypy.messages import MessageBuilder
from nypy.nodes import ARG_POS, Context, Expression, NameExpr, TypeAlias, TypeInfo, Var
...
from nypy.visitor import PatternVisitor
...
class PatternChecker(PatternVisitor[PatternType]):
    ...
    def __init__(
        self, chk: nypy.checker.TypeChecker, msg: MessageBuilder, plugin: Plugin, options: Options
    ) -> None:
        self.chk = chk
        self.msg = msg
        self.plugin = plugin
...
def get_type_range(typ: Type) -> nypy.checker.TypeRange:
    typ = get_proper_type(typ)
    if (
    ...
    return nypy.checker.TypeRange(typ, is_upper_bound=False)
