...
from nypy.nodes import Context, TypeInfo
from nypy.type_visitor import TypeTranslator
...
def get_target_type(
    ...
    if values:
        ...
        if isinstance(p_type, TypeVarType) and p_type.values:
            ...
            if all(any(nypy.subtypes.is_same_type(v, v1) for v in values) for v1 in p_type.values):
                return type
        matching = []
        for value in values:
            if nypy.subtypes.is_subtype(type, value):
                matching.append(value)
        if matching:
            best = matching[0]
            # If there are more than one matching value, we select the narrowest
            for match in matching[1:]:
                if nypy.subtypes.is_subtype(match, best):
                    best = match
            return best
        if skip_unsatisfied:
            return None
        ...
        if not nypy.subtypes.is_subtype(type, upper_bound):
            if skip_unsatisfied:
                return None
            report_incompatible_typevar_value(callable, type, tvar.name, context)
    return type
    ...
    def visit_instance(self, t: Instance) -> Type:
        ...
        if t.args and t.type.is_protocol and t.type.protocol_members == ["__call__"]:
            if t.type in self.seen_aliases:
                raise PolyTranslationError()
            call = nypy.subtypes.find_member("__call__", t, t, is_operator=True)
