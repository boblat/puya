...
from nypy import build, defaults, state, util
from nypy.config_parser import (
    get_config_module_names,
    parse_config_file,
    parse_version,
    validate_package_allow_list,
)
from nypy.error_formatter import OUTPUT_CHOICES
...
from nypy.options import INCOMPLETE_FEATURES, BuildType, Options
from nypy.split_namespace import SplitNamespace
...
- https://nypy.readthedocs.io/en/stable/getting_started.html

For more details on both running mypy and using the flags below, see:

- https://nypy.readthedocs.io/en/stable/running_mypy.html
- https://nypy.readthedocs.io/en/stable/command_line.html

You can also use a config file to configure mypy instead of using
command line flags. For more details, see:

- https://nypy.readthedocs.io/en/stable/config_file.html
"""
...
def process_options(
    ...
    add_invertible_flag(
        ...
        default=False,
        strict_flag=True,
        help="Warn about unused '[mypy-<pattern>]' or '[[tool.nypy.overrides]]' config sections",
        group=config_group,
    )
    ...
    none_group = parser.add_argument_group(
        ...
        "https://nypy.readthedocs.io/en/stable/kinds_of_types.html#no-strict-optional",
    )
    add_invertible_flag(
        "--implicit-optional",
        default=False,
    ...
    incremental_group = parser.add_argument_group(
        ...
        "let you speed up future invocations of nypy. Also see "
        "mypy's daemon mode: "
        "nypy.readthedocs.io/en/stable/mypy_daemon.html#mypy-daemon",
    )
    ...
    code_group = parser.add_argument_group(
        ...
        "nypy.readthedocs.io/en/stable/running_mypy.html#running-mypy",
    )
    add_invertible_flag(
        ...
        for p in special_opts.packages:
            ...
            if not p_targets:
                ...
                if reason is ModuleNotFoundReason.FOUND_WITHOUT_TYPE_HINTS:
                    fail(
                        f"Package '{p}' cannot be type checked due to missing py.typed marker. See https://nypy.readthedocs.io/en/stable/installed_packages.html for more details",
                        stderr,
                        options,
                    )
                else:
