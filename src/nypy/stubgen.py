...
import nypy.build
import nypy.mixedtraverser
import nypy.parse
import nypy.traverser
import nypy.util
import nypy.version
from nypy.build import build
from nypy.errors import CompileError, Errors
from nypy.find_sources import InvalidSourceList, create_source_list
...
from nypy.moduleinspect import ModuleInspect, is_pyc_only
...
from nypy.options import Options as MypyOptions
from nypy.plugins.dataclasses import DATACLASS_FIELD_SPECIFIERS
from nypy.semanal_shared import find_dataclass_transform_spec
from nypy.sharedparse import MAGIC_METHODS_POS_ARGS_ONLY
from nypy.stubdoc import ArgSig, FunctionSig
from nypy.stubgenc import InspectionStubGenerator, generate_stub_for_c_module
from nypy.stubutil import (
...
class DefinitionFinder(nypy.traverser.TraverserVisitor):
...
class ReferenceFinder(nypy.mixedtraverser.MixedTraverserVisitor):
...
class ASTStubGenerator(BaseStubGenerator, nypy.traverser.TraverserVisitor):
    ...
    def visit_class_def(self, o: ClassDef) -> None:
        ...
        if self._include_docstrings and o.docstring:
            docstring = nypy.util.quote_docstring(o.docstring)
    ...
    def visit_import_from(self, o: ImportFrom) -> None:
        ...
        if self.module_name:
            full_module, ok = nypy.util.correct_relative_import(
...
class SelfTraverser(nypy.traverser.TraverserVisitor):
    def __init__(self) -> None:
...
def find_module_paths_using_search(
    ...
    typeshed_path = default_lib_path(nypy.build.default_data_dir(), pyversion, None)
...
def parse_source_file(mod: StubSource, mypy_options: MypyOptions) -> None:
    ...
    with open(mod.path, "rb") as f:
        data = f.read()
    source = nypy.util.decode_python_encoding(data)
    errors = Errors(mypy_options)
    mod.ast = nypy.parse.parse(
...
def main(args: list[str] | None = None) -> None:
    nypy.util.check_python_version("stubgen")
