...
import nypy.types
import nypy.version
from nypy import nodes
from nypy.config_parser import parse_config_file
from nypy.evalexpr import UNKNOWN, evaluate_expression
from nypy.options import Options
from nypy.util import FancyFormatter, bytes_to_human_readable_repr, is_dunder, plural_s
...
def _verify_metaclass(
    ...
    if not stub.is_protocol and not stub.is_enum and not is_runtime_typeddict:
        ...
        if runtime_metaclass is not type and stub.metaclass_type is None:
            ...
            and not nypy.types.is_named_instance(
...
def _verify_arg_default_value(
    ...
    if runtime_arg.default is not inspect.Parameter.empty:
        if stub_arg.kind.is_required():
            ...
            if isinstance(stub_type, nypy.types.TypeVarType):
...
class Signature(Generic[T]):
    ...
    def from_overloadedfuncdef(stub: nodes.OverloadedFuncDef) -> Signature[nodes.Argument]:
        ...
        def get_type(arg_name: str) -> nypy.types.ProperType:
            with nypy.state.state.strict_optional_set(True):
                all_types = [
                    arg.variable.type or arg.type_annotation for arg, _ in all_args[arg_name]
                ]
                return nypy.typeops.make_simplified_union([t for t in all_types if t])
...
def verify_var(
    ...
    if (
        ...
        if isinstance(runtime, enum.Enum):
            ...
            proper_type = nypy.types.get_proper_type(stub.type)
            if (
                isinstance(proper_type, nypy.types.Instance)
...
def _verify_readonly_property(stub: nodes.Decorator, runtime: Any) -> Iterator[str]:
    ...
    runtime_type = get_mypy_type_of_runtime_value(runtime)
    func_type = (
        stub.func.type.ret_type if isinstance(stub.func.type, nypy.types.CallableType) else None
    )
    if (
        runtime_type is not None
...
def _resolve_funcitem_from_decorator(dec: nodes.OverloadPart) -> nodes.FuncItem | None:
    ...
    def apply_decorator_to_funcitem(
        ...
        if (
            ...
            and decorator.callee.fullname in nypy.types.DEPRECATED_TYPE_NAMES
        ):
            return func
        ...
        if (
            ...
            or decorator.fullname in nypy.types.OVERLOAD_NAMES
            or decorator.fullname in nypy.types.FINAL_DECORATOR_NAMES
        ):
            return func
...
def verify_typealias(
    ...
    stub_target = nypy.types.get_proper_type(stub.target)
    ...
    if isinstance(stub_target, nypy.types.Instance):
        ...
        return
    if isinstance(stub_target, nypy.types.UnionType):
        ...
        return
    if isinstance(stub_target, nypy.types.TupleType):
        ...
        return
    if isinstance(stub_target, nypy.types.CallableType):
        ...
        return
    if isinstance(stub_target, nypy.types.AnyType):
        return
...
def is_subtype_helper(left: nypy.types.Type, right: nypy.types.Type) -> bool:
    """Checks whether ``left`` is a subtype of ``right``."""
    left = nypy.types.get_proper_type(left)
    right = nypy.types.get_proper_type(right)
    if (
        isinstance(left, nypy.types.LiteralType)
        and isinstance(left.value, int)
        and left.value in (0, 1)
        and nypy.types.is_named_instance(right, "builtins.bool")
    ):
        # Pretend Literal[0, 1] is a subtype of bool to avoid unhelpful errors.
        return True

    if isinstance(right, nypy.types.TypedDictType) and nypy.types.is_named_instance(
        left, "builtins.dict"
    ):
    ...
    with nypy.state.state.strict_optional_set(True):
        return nypy.subtypes.is_subtype(left, right)


def get_mypy_type_of_runtime_value(runtime: Any) -> nypy.types.Type | None:
    ...
    """
    if runtime is None:
        return nypy.types.NoneType()
    if isinstance(runtime, property):
        # Give up on properties to avoid issues with things that are typed as attributes.
        return None

    def anytype() -> nypy.types.AnyType:
        return nypy.types.AnyType(nypy.types.TypeOfAny.unannotated)
    ...
    if isinstance(
        ...
        fallback = nypy.types.Instance(type_info, [anytype()])
        signature = safe_inspect_signature(runtime)
        if signature:
            arg_types = []
            arg_kinds = []
        ...
        return nypy.types.CallableType(
            arg_types,
            arg_kinds,
            arg_names,
            ret_type=anytype(),
    ...
    if isinstance(runtime, tuple):
        ...
        fallback = nypy.types.Instance(type_info, [anytype()])
        return nypy.types.TupleType(items, fallback)

    fallback = nypy.types.Instance(type_info, [anytype() for _ in type_info.type_vars])
    ...
    return nypy.types.LiteralType(value=value, fallback=fallback)
...
def build_stubs(modules: list[str], options: Options, find_submodules: bool = False) -> list[str]:
    ...
    """
    data_dir = nypy.build.default_data_dir()
    search_path = nypy.modulefinder.compute_search_paths([], options, data_dir)
    find_module_cache = nypy.modulefinder.FindModuleCache(
    ...
    for module in modules:
        ...
        if not find_submodules:
            ...
            if not isinstance(module_path, str):
                ...
                continue
            sources.append(nypy.modulefinder.BuildSource(module_path, module, None))
        else:
    ...
    if sources:
        try:
            res = nypy.build.build(sources=sources, options=options)
        except nypy.errors.CompileError as e:
...
def get_typeshed_stdlib_modules(
    ...
    stdlib_py_versions = nypy.modulefinder.load_stdlib_py_versions(custom_typeshed_dir)
    ...
    if custom_typeshed_dir:
        ...
        typeshed_dir = Path(nypy.build.default_data_dir()) / "typeshed"
...
def parse_options(args: list[str]) -> _Arguments:
    ...
    parser.add_argument(
        "--version", action="version", version="%(prog)s " + nypy.version.__version__
    )
...
def main() -> int:
    nypy.util.check_python_version("stubtest")
