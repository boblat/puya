...
import nypy.applytype
import nypy.constraints
import nypy.typeops
from nypy.erasetype import erase_type
...
from nypy.types_utils import flatten_types
from nypy.typestate import SubtypeKind, type_state
from nypy.typevars import fill_typevars, fill_typevars_with_any
...
def is_subtype(
    ...
    *,
    ...
    if type_state.is_assumed_subtype(left, right):
        return True
    if nypy.typeops.is_recursive_pair(left, right):
...
def _is_subtype(
    ...
    if isinstance(right, UnionType) and not isinstance(left, UnionType):
        ...
        if (
            ...
            right = UnionType(nypy.typeops.try_contracting_literals_in_union(right.items))
            if proper_subtype:
                is_subtype_of_item = any(
                    is_proper_subtype(orig_left, item, subtype_context=subtype_context)
                    for item in right.items
...
class SubtypeVisitor(TypeVisitor[bool]):
    ...
    def visit_instance(self, left: Instance) -> bool:
        ...
        if isinstance(right, TupleType) and right.partial_fallback.type.is_enum:
            return self._is_subtype(left, nypy.typeops.tuple_fallback(right))
        ...
        if isinstance(right, TypeType):
            ...
            if isinstance(item, TupleType):
                item = nypy.typeops.tuple_fallback(item)
    ...
    def visit_tuple_type(self, left: TupleType) -> bool:
        ...
        if isinstance(right, Instance):
            if is_named_instance(right, "typing.Sized"):
                ...
                nypy.typeops.tuple_fallback(left), right
            ):
                return True
            return False
    ...
    def visit_union_type(self, left: UnionType) -> bool:
        if isinstance(self.right, Instance):
            ...
            for item in left.relevant_items():
                ...
                lit_type = nypy.typeops.simple_literal_type(p_item)
...
def is_protocol_implementation(
    ...
    with pop_on_exit(assuming, left, right):
        for member in right.type.protocol_members:
            ...
            subtype = nypy.typeops.get_protocol_member(left, member, class_obj)
...
def find_node_type(
    ...
    """
    from nypy.typeops import bind_self

    if isinstance(node, FuncBase):
        typ: Type | None = nypy.typeops.function_type(
...
def unify_generic_callable(
    ...
    if set(type.type_var_ids()) & {v.id for v in nypy.typeops.get_all_type_vars(target)}:
    ...
    if return_constraint_direction is None:
        return_constraint_direction = nypy.constraints.SUBTYPE_OF

    constraints: list[nypy.constraints.Constraint] = []
    # There is some special logic for inference in callables, so better use them
    # as wholes instead of picking separate arguments.
    cs = nypy.constraints.infer_constraints(
        type.copy_modified(ret_type=UninhabitedType()),
        target.copy_modified(ret_type=UninhabitedType()),
        nypy.constraints.SUBTYPE_OF,
        skip_neg_op=True,
    )
    constraints.extend(cs)
    if not ignore_return:
        c = nypy.constraints.infer_constraints(
            type.ret_type, target.ret_type, return_constraint_direction
        )
        constraints.extend(c)
    inferred_vars, _ = nypy.solve.solve_constraints(
    ...
    applied = nypy.applytype.apply_generic_arguments(
...
def try_restrict_literal_union(t: UnionType, s: Type) -> list[Type] | None:
    ...
    if not nypy.typeops.is_simple_literal(ps):
        return None

    new_items: list[Type] = []
    for i in t.relevant_items():
        pi = get_proper_type(i)
        if not nypy.typeops.is_simple_literal(pi):
            return None
        if pi != ps:
            new_items.append(i)
    return new_items
    ...
    for variance in COVARIANT, CONTRAVARIANT, INVARIANT:
        ...
        for member in all_non_object_members(info):
            ...
            if isinstance(self_type, TupleType):
                self_type = nypy.typeops.tuple_fallback(self_type)
